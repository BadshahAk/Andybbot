...
from TeamLegend.Config import LEGEND

from TeamLegend.helpers.LEGEND import IMMUNE_USERS
from TeamLegend.core.clients import LEGEND
...
async def user_is_admin(user_id: int, message):
    ...
    async for user in telethn.iter_participants(
        ...
        if user_id == user.id or user_id in LEGEND:
            status = True
            break
    return status
...
async def KannadigaBot_is_admin(chat_id: int):
    status = False
    kannadiga = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if kannadiga.id == user.id:
            status = True
            break
    return status
