...
from TeamLegend.Config import EVENT_LOGS, SUPPORT_CHAT
from TeamLegend.core.clients import dispatcher, LOGS
...
# Do not async
@connection_status
def get(bot, update, context, notename, show_none=True, no_format=False):
    bot = context.bot
...
def clearall(update: Update, context: CallbackContext):
    ...
    if member.status != "creator" and user.id not in LEGEND:
...
def clearall_btn(update: Update, context: CallbackContext):
    ...
    if query.data == "notes_rmall":
        if member.status == "creator" or query.from_user.id in LEGEND:
